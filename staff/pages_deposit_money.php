...
if (isset($_POST['deposit'])) {
    ...
    $current_balance_row = $balance_result->fetch_assoc();
    $current_balance = $current_balance_row['acc_amount'];

    // Calculate new balance
    $updated_balance = $current_balance + $transaction_amt;

    // Notification
    $notification_details = "$client_name Has Deposited $$transaction_amt To Bank Account $account_number";

    // ✅ Updated INSERT with 16 fields
    $query = "INSERT INTO iB_Transactions (
        tr_code, account_id, acc_name, account_number, acc_type,
        tr_type, tr_status, client_id, client_name, client_national_id,
        transaction_amt, client_phone, acc_amount, receiving_acc_no,
        receiving_acc_name, receiving_acc_holder
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    $stmt->bind_param(
        ...
        $tr_code, $account_id, $acc_name, $account_number, $acc_type,
        $tr_type, $tr_status, $client_id, $client_name, $client_national_id,
        $transaction_amt, $client_phone, $updated_balance, $account_number,
        $acc_name, $acc_name // assuming holder = acc_name
    );
    ...
    $update_stmt->bind_param("di", $updated_balance, $account_id);

    // Execute all
    if ($stmt->execute() && $notification_stmt->execute() && $update_stmt->execute()) {
